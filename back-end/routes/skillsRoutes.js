...
SkillsRoutes.get("/:className/:spec?", async (req, res) => {
  ...
  await db
    .transaction(async (trx) => {
      ...
      const skillsHitsQuery = await trx("skills_hits")
        .whereIn(
          "skill_id",
          skillsQuery.map((skill) => skill.skill_id),
        )
        .orderBy(["skill_id", "hit_count"]);
...
SkillsRoutes.post("/", ensureAdmin, async (req, res) => {
  ...
  await db
    .transaction(async (trx) => {
      const skill_already_exists = await trx("skills")
...
const download_icon = async (className, id) => {
  ...
  try {
    ...
    const img_url =
      icon_base_url + final_url.data.data.icon_image.toLowerCase() + ".webp";
    ...
    const web_path = await new s3Service().uploadFile(
      `skills/${className}/icon_${id}.webp`,
      body,
      "image/webp",
    );
