  ...
  <div class="admin-layout">
    <aside class="menu">
      ...
      <ul class="menu-list">
        <li>
          <a
            @click="chooseMenuItem('dashboard')"
            :class="{ 'is-active': showDashboard }"
            >Dashboard</a
          >
        </li>
      </ul>
      ...
      <ul class="menu-list">
        <li>
          <a
            @click="(chooseMenuItem('Calculator Presets'), getAllPresets())"
            :class="{ 'is-active': showCalculatorPresets }"
            >Calculator Presets</a
          >
        </li>
        <li>
          <a
            @click="chooseMenuItem('Skills Management')"
            :class="{ 'is-active': showSkillsManagement }"
            >Skills Management</a
          >
        </li>
      </ul>
    ...
    <div class="content">
      ...
      <div class="admin-main-content">
        <div v-if="showSkillsManagement">
          ...
          <div>
            <SkillsManager :selectedClass="'hashashin'" />
          </div>
        </div>
        <div v-if="showCalculatorPresets">
          ...
          <nav class="mt-5 panel">
            ...
            <a
              v-for="preset in allCalculatorPresets"
              :key="preset.id"
              class="panel-block"
            >
              ...
              <div>
                <strong>{{ preset.name }} | ID: {{ preset.id }}</strong>
              ...
              <div class="is-flex ml-auto">
                <button
                  class="button is-white is-outlined mr-1"
          ...
          <div
            ...
            class="notification"
            :class="deleteError ? 'is-danger' : 'is-primary'"
          >
            <button
              class="delete"
              @click="showDeleteNotification = !showDeleteNotification"
            ></button>
            {{ deleteMessage }}
          </div>
        </div>
  ...
  setup() {
    ...
    const deletePreset = async (id) => {
      try {
        ...
        const response = await apiClient.delete(
          `/pvp-calculator/presets/${id}`,
        );
        deleteMessage.value = response.data.message;
        if (response.data.success) {
          let filterById = (preset) => preset.id !== id;
          allCalculatorPresets.value =
            allCalculatorPresets.value.filter(filterById);
        }
      } catch (error) {
    ...
    const savePreset = async (presetData) => {
      try {
        ...
        if (presetData.id) {
          ...
          ((method = "post"), (url = "/pvp-calculator/presets"));
        ...
        if (method === "post") {
          ...
          const preset = allCalculatorPresets.value.find(
    ...
    const getPresetById = async (id) => {
      try {
        ...
        if (response.data.success) return response.data.preset;
      } catch (error) {
